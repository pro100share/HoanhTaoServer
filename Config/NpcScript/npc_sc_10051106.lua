...
function npc_fun.cb_npc_on_opera(objNpc,objPlayer,sysNpc,sysScript)
	...
	if sysScript:AddTaskItem(7100153,1,enItemBindType.eYes) 	then 
	local Task = sysTask:GetTask(4002)		
		if Task:GetTaskState()~=TaskStateConfig.Done then
				sysScript:MovePlayer(-9,48,nil)
   end					
	else
