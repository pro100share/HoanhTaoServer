...
function npc_fun.cb_npc_on_opera(objNpc,objPlayer,sysNpc,sysScript)
	...
	if sysScript:AddTaskItem(7100038,1,enItemBindType.eYes) 	then 
	local Task = sysTask:GetTask(4026)		
		if Task:GetTaskState()~=TaskStateConfig.Done then
				sysScript:MovePlayer(-12,-22,nil)
   end				
	else
