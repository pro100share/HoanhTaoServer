...
function npc_fun.cb_npc_on_opera(objNpc,objPlayer,sysNpc,sysScript)
	...
	if sysScript:AddTaskItem(7100137,1,enItemBindType.eYes) 	then 
	local Task = sysTask:GetTask(5010)	
		if Task:GetTaskState()~=TaskStateConfig.Done then
				sysScript:MovePlayer(-22,98,nil)	
		end	
	else 
