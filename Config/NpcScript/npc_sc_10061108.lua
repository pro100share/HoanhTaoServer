...
function npc_fun.cb_npc_on_opera(objNpc,objPlayer,sysNpc,sysScript)
	...
	if sysScript:AddTaskItem(7100139,1,enItemBindType.eYes) 	then 
	local Task = sysTask:GetTask(5027)		
		if Task:GetTaskState()~=TaskStateConfig.Done then
				sysScript:MovePlayer(-22,99,nil)
   end				
	else 
