...
function npc_fun.cb_npc_on_opera(objNpc,objPlayer,sysNpc,sysScript)
	...
	if sysScript:AddTaskItem(7100142,1,enItemBindType.eYes) 	then 
	local Task = sysTask:GetTask(5037)		
		if Task:GetTaskState()~=TaskStateConfig.Done then
				sysScript:MovePlayer(-33,74,nil)
   end			
	else 
