local dwMonsterId = 50302022	--纪无双
local MonsterScript = {}
------
--[[
--]]
...
function MonsterScript:CBMonsterOnDead(Monster,objPlayer)
	...
	local Hp , _ = ZhenlongChessSys:GetZhenlongInfo()
	local LostHPStar = {
		{45000,5},
		{50000,4},
		{53000,3},
	...
	for i = 1 , #LostHPStar do
		if Hp < LostHPStar[i][1] then
			if ZhenlongChessSys:GetBossStarLevel(dwMonsterId) < LostHPStar[i][2] then
				ZhenlongChessSys:SetBossStarLevel(dwMonsterId, LostHPStar[i][2])
			end
		end
	end
	ZhenlongChessSys:SetProcess(23)
	ScriptSys:AddSpItem(1,5000)
	ScriptSys:AddSpItem(3,10000)
	-- MonsterDecision:LostHP(Monster,dwMonsterId,objPlayer)
end
---------------------------------------------
--受到攻击时执行
