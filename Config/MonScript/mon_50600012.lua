...
function MonsterScript:CBMonsterOnDead(Monster,objPlayer,sysScript)
	n = n+1
	if n==35 then
		Monster:SetDuplData(13)
		Monster:MapNotice(9000660813,7)	
		n = 0
	end 
	
	local objMap = Monster.Map
	objMap.qjCount = objMap.qjCount or {}
	objMap.qjCount[50600012] = (objMap.qjCount[50600012] or 0) + 1
	...
	if (objMap.qjCount[50600012] == 35) then
		local BoShu = 13;
...
--受到攻击时执行
function MonsterScript:CBMonsterOnAttacked(Monster,objPlayer,id,dwHurtValue)
	local lev = Monster:GetLevel()
	local Value = math.floor((lev/10)*50+(dwHurtValue*0.03))
	local objDefBattle = objPlayer:GetSystem("CBattleSystem");
	objDefBattle:Hurt(-Value,false,Monster,0,0,0,0,true);
end
--怪物出生时执行
