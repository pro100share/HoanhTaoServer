local dwMonsterId = 50720003	--BOSS
local MonsterScript = {}
------
--[[
--]]
...
function MonsterScript:CBMonsterOnUpdate(Monster)
	...
	if (Pos.x+2.31)^2+(Pos.y+2.49)^2 < 1^2 then
		...
		Map.xiayibo03 = (Map.xiayibo03 or 0) + 1
		...
		if  Map.xiayibo03 == 80 then
			...
			local D1 = function()
				...
				local dwRand = math.random(1000);
					...
					if (dwRand < 500) then
						Monster:MapNotice(9000660560,7)
						for sg = 1,40 do  ----刷第四波怪1  金
							mgrMonster:DelayAdd(50720004,(1000+dwLevelm+1),50720001,50720001,100320040,-61.85,67.30,dwObjId,(sg*1500))
						end
						for sg = 1,40 do  ----刷第四波怪2 南疆
							mgrMonster:DelayAdd(50720004,(1000+dwLevelm+1),50720001,50720001,100520010,-68.14,-63.61,dwObjId,(sg*1500))
						end
						mgrMonster:DelayAdd(50720014,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100520020,-61.85,67.30,dwObjId,60000)--出BOSS
						
					else
						Monster:MapNotice(9000660561,7)
						for sg = 1,40 do  ----刷第四波怪3  波斯
							mgrMonster:DelayAdd(50720004,(1000+dwLevelm+1),50720001,50720001,101120040,73.90,-80.36,dwObjId,(sg*1500))
						end	
						for sg = 1,40 do  ----刷第四波怪4 蒙古
							mgrMonster:DelayAdd(50720004,(1000+dwLevelm+1),50720001,50720001,100920020,72.66,55.16,dwObjId,(sg*1500))
						end	
						mgrMonster:DelayAdd(50720014,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100710100,73.90,-80.36,dwObjId,60000)--出BOSS
			...
			local D1 = function()
				...
				local dwRand = math.random(1000);
					...
					if (dwRand < 500) then
						...
						mgrMonster:DelayAdd(50720014,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100420030,-68.14,-63.61,dwObjId,60000)--出BOSS
						...
						mgrMonster:DelayAdd(50720014,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100420020,72.66,55.16,dwObjId,60000)--出BOSS
