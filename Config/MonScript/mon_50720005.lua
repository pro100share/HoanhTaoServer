local dwMonsterId = 50720005	--BOSS
local MonsterScript = {}
------
--[[
--]]
...
function MonsterScript:CBMonsterOnUpdate(Monster)
	...
	if (Pos.x+2.31)^2+(Pos.y+2.49)^2 < 1^2 then
		...
		Map.xiayibo05 = (Map.xiayibo05 or 0) + 1
		...
		if  Map.xiayibo05 == 135 then
			...
			local D1 = function()
				...
				local dwRand = math.random(1000);
					...
					if (dwRand < 500) then
						Monster:MapNotice(9000660564,7)
						for sg = 1,45 do  ----刷第六波怪1  金
							mgrMonster:DelayAdd(50720006,(1000+dwLevelm+1),50720001,50720001,100120050,-61.85,67.30,dwObjId,(sg*1500))
						end
						for sg = 1,45 do  ----刷第六波怪2 南疆
							mgrMonster:DelayAdd(50720006,(1000+dwLevelm+1),50720001,50720001,503020070,-68.14,-63.61,dwObjId,(sg*1500))
						end
						for sg = 1,45 do  ----刷第六波怪3  波斯
							mgrMonster:DelayAdd(50720006,(1000+dwLevelm+1),50720001,50720001,101120050,73.90,-80.36,dwObjId,(sg*1500))
						end	
						mgrMonster:DelayAdd(50720016,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100120040,-61.85,67.30,dwObjId,67500)--出BOSS
						
					else
						Monster:MapNotice(9000660565,7)
						for sg = 1,45 do  ----刷第六波怪3  波斯
							mgrMonster:DelayAdd(50720006,(1000+dwLevelm+1),50720001,50720001,101120050,73.90,-80.36,dwObjId,(sg*1500))
						end	
						for sg = 1,45 do  ----刷第六波怪4 蒙古
							mgrMonster:DelayAdd(50720006,(1000+dwLevelm+1),50720001,50720001,100720020,72.66,55.16,dwObjId,(sg*1500))
						end	
						for sg = 1,45 do  ----刷第六波怪1  金
							mgrMonster:DelayAdd(50720006,(1000+dwLevelm+1),50720001,50720001,100120050,-61.85,67.30,dwObjId,(sg*1500))
						end
						mgrMonster:DelayAdd(50720016,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100710110,73.90,-80.36,dwObjId,67500)--出BOSS
			...
			local D1 = function()
				...
				local dwRand = math.random(1000);
					...
					if (dwRand < 500) then
						...
						mgrMonster:DelayAdd(50720016,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),503020140,-68.14,-63.61,dwObjId,67500)--出BOSS
						...
						mgrMonster:DelayAdd(50720016,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100920090,72.66,55.16,dwObjId,67500)--出BOSS
