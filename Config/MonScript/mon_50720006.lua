local dwMonsterId = 50720006	--BOSS
local MonsterScript = {}
------
--[[
--]]
...
function MonsterScript:CBMonsterOnUpdate(Monster)
	...
	if (Pos.x+2.31)^2+(Pos.y+2.49)^2 < 1^2 then
		...
		Map.xiayibo06 = (Map.xiayibo06 or 0) + 1
		...
		if  Map.xiayibo06 == 135 then
				
			local D1 = function()
					Monster:MapNotice(9000660566,7)
						for sg = 1,45 do  ----刷第六波怪1  金
							mgrMonster:DelayAdd(50720007,(1000+dwLevelm+1),50720001,50720001,100120040,-61.85,67.30,dwObjId,(sg*1500))
						end
						for sg = 1,45 do  ----刷第六波怪2 南疆
							mgrMonster:DelayAdd(50720007,(1000+dwLevelm+1),50720001,50720001,503020140,-68.14,-63.61,dwObjId,(sg*1500))
						end
						for sg = 1,45 do  ----刷第六波怪3  波斯
							mgrMonster:DelayAdd(50720007,(1000+dwLevelm+1),50720001,50720001,100710110,73.90,-80.36,dwObjId,(sg*1500))
						end	
						for sg = 1,45 do  ----刷第六波怪4 蒙古
							mgrMonster:DelayAdd(50720007,(1000+dwLevelm+1),50720001,50720001,100920090,72.66,55.16,dwObjId,(sg*1500))
						end	
						
					local dwRand = math.random(1000);
					if (dwRand < 500) then
						mgrMonster:DelayAdd(50720017,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100720030,-61.85,67.30,dwObjId,67500)--出BOSS
						mgrMonster:DelayAdd(50720017,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100620060,-68.14,-63.61,dwObjId,67500)--出BOSS
					else
						mgrMonster:DelayAdd(50720017,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),101120060,73.90,-80.36,dwObjId,67500)--出BOSS
						mgrMonster:DelayAdd(50720017,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100820050,72.66,55.16,dwObjId,67500)--出BOSS
					end
