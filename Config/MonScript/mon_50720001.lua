local dwMonsterId = 50720001	--BOSS
local MonsterScript = {}
------
--[[
--]]
...
function MonsterScript:CBMonsterOnUpdate(Monster)
	...
	if (Pos.x+2.31)^2+(Pos.y+2.49)^2 < 1^2 then
		...
		Map.xiayibo01 = (Map.xiayibo01 or 0) + 1
			...
			local D1 = function()
				...
				local dwRand = math.random(1000);
					...
					if (dwRand < 500) then
						...
						for sg = 1,55 do  ----刷第二波怪1  金
							mgrMonster:DelayAdd(50720002,(1000+dwLevelm+1),50720001,50720001,100320010,-61.85,67.30,dwObjId,(sg*1500))
						end		----刷第二波怪1
						mgrMonster:DelayAdd(50720012,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100320030,-61.85,67.30,dwObjId,82500)--出BOSS
						
					else
						Monster:MapNotice(9000660557,7)
						for sg = 1,55 do  ----刷第二波怪2  波斯
							mgrMonster:DelayAdd(50720002,(1000+dwLevelm+1),50720001,50720001,100710080,73.90,-80.36,dwObjId,(sg*1500))
						end		----刷第二波怪2
						mgrMonster:DelayAdd(50720012,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),101020060,73.90,-80.36,dwObjId,82500)--出BOSS
