local dwMonsterId = 50720002	--BOSS
local MonsterScript = {}
------
--[[
--]]
...
function MonsterScript:CBMonsterOnBorn(Monster)
	...
	local Pos = Monster:GetVectorPos()		---获得怪物坐标
		...
		if (Pos.x+61.85)^2+(Pos.y-67.30)^2 < 2^2 then
...
function MonsterScript:CBMonsterOnUpdate(Monster)
	...
	if (Pos.x+2.31)^2+(Pos.y+2.49)^2 < 1^2 then
		...
		Map.xiayibo02 = (Map.xiayibo02 or 0) + 1
		...
		if  Map.xiayibo02 == 55 then
			...
			local D1 = function()
				...
				local dwRand = math.random(1000);
					...
					if (dwRand < 500) then
						Monster:MapNotice(9000660558,7)
						for sg = 1,40 do  ----刷第三波怪1  金
							mgrMonster:DelayAdd(50720003,(1000+dwLevelm+1),50720001,50720001,100320030,-61.85,67.30,dwObjId,(sg*1500))
						end
						for sg = 1,40 do  ----刷第三波怪2 蒙古
							mgrMonster:DelayAdd(50720003,(1000+dwLevelm+1),50720001,50720001,101120020,72.66,55.16,dwObjId,(sg*1500))
						end	
						mgrMonster:DelayAdd(50720013,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100320040,-61.85,67.30,dwObjId,60000)--出BOSS
						
					else
						Monster:MapNotice(9000660559,7)
						for sg = 1,40 do  ----刷第三波怪3 南疆
							mgrMonster:DelayAdd(50720003,(1000+dwLevelm+1),50720001,50720001,100620010,-68.14,-63.61,dwObjId,(sg*1500))
						end
						for sg = 1,40 do  ----刷第三波怪4  波斯
							mgrMonster:DelayAdd(50720003,(1000+dwLevelm+1),50720001,50720001,101020060,73.90,-80.36,dwObjId,(sg*1500))
						end	
						mgrMonster:DelayAdd(50720013,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100320030,-68.14,-63.61,dwObjId,60000)--出BOSS
	...
	Map.xiayibo02 = (Map.xiayibo02 or 0) + 1
		...
		if  Map.xiayibo02 == 55 then
			...
			local D1 = function()
				...
				local dwRand = math.random(1000);
						--Monster:MapNotice(9000660556,7)
					if (dwRand < 500) then
						Monster:MapNotice(9000660558,7)
						for sg = 1,40 do  ----刷第三波怪1  金
							mgrMonster:DelayAdd(50720003,(1000+dwLevelm+1),50720001,50720001,100320030,-61.85,67.30,dwObjId,(sg*1500))
						...
						mgrMonster:DelayAdd(50720013,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),100920020,72.66,55.16,dwObjId,60000)--出BOSS
						...
						mgrMonster:DelayAdd(50720013,(3000+dwLevelm+5),50720011,(102000+(math.floor(dwLevelm/5)*5)),101120040,73.90,-80.36,dwObjId,60000)--出BOSS
