local dwMonsterId = 50702002  -------本波8辆车
local MonsterScript = {}
------
--[[
--]]
...
function MonsterScript:CBMonsterOnUpdate(Monster)
	...
	local dwLevelm = Monster:GetLevel()        ----获得怪物等级
		...
		if (Pos.x-32.62)^2+(Pos.y+29.02)^2 < 2^2 then
			...
			if Map.lrxxx>5 then
				local Monster01 = mgrMonster:GetMonsterById(50702002)
				if #Monster01<1 then
					Map.lrxxx = 0	---地图计数器清0
					...
					local C1 = function()
					mgrMonster:Add(50702003,(1000+dwLevelm+1),50702003,50702001,80010,-40.75,49.73)
					...
					mgrMonster:DelayAdd(50702003,(1000+dwLevelm+1),50702003,50702001,80010,-40.75,49.73,dwObjId,10500)
					...
					mgrMonster:DelayAdd(50702003,(1000+dwLevelm+1),50702003,50702001,80010,-40.75,49.73,dwObjId,21000)
					...
					mgrMonster:DelayAdd(50702003,(1000+dwLevelm+1),50702003,50702001,80010,-40.75,49.73,dwObjId,31500)
					...
					mgrMonster:DelayAdd(50702003,(1000+dwLevelm+1),50702003,50702001,80010,-40.75,49.73,dwObjId,42000)
					...
					mgrMonster:DelayAdd(50702003,(1000+dwLevelm+1),50702003,50702003,80020,-40.75,49.73,dwObjId,52500)
					...
					mgrMonster:DelayAdd(50702003,(1000+dwLevelm+1),50702003,50702003,80020,-40.75,49.73,dwObjId,63000)
					...
					mgrMonster:DelayAdd(50702003,(1000+dwLevelm+1),50702004,50702003,80020,-40.75,49.73,dwObjId,73500)
...
function MonsterScript:CBMonsterOnDead(Monster,objPlayer,sysScript)
	...
	--if Map.lrxxx>6 then
		local Monster01 = mgrMonster:GetMonsterById(50702002)
		if #Monster01<1 and  Map.lrxxx>5 then
					Map.lrxxx = 0	---地图计数器清0
					Map.shijian = 0		---特殊事件计数器清0
					Map.shijian02 = 0	---特殊事件02计数器清0 ----用来防止第1个触发点多次触发
					Map.shijian03 = 0	---特殊事件03计数器清0 ----用来防止第2个触发点多次触发
