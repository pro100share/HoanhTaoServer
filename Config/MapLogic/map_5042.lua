...
function MapLogic:EventPlayerEnter(map,objPlayer)
	...
	local D1 = function()
		...
		--播动画 刷怪
		sysScript:Story(5040002);
		mgrMonster:DelayAdd(51000009, 51000009, 51000001, 51000009, 100620060, -1, 13, dwObjId, 2500);
		for i = 1, #pos1 do
			mgrMonster:DelayAdd(51000008, 51000008, 51000001, 101300, 100820040, pos1[i].x, pos1[i].y, dwObjId, 2000);
		end
		
		for i = 1, #pos2 do
			mgrMonster:DelayAdd(51000007, 51000007, 51000001, 101300, 100620010, pos2[i].x, pos2[i].y, dwObjId, 2000);
		end
	end
...
function MapLogic:RemoveMon(map)
	local mgrMonster = map:GetMonsterMgr();		----获得怪物管理器
	local monster = {51000007, 51000008, 51000009};
	for i = 1, #monster do
