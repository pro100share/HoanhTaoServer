...
function MapLogic:EventPlayerEnter(map,objPlayer)
	...
	local D1 = function()
		...
		--播动画 刷怪
		sysScript:Story(5040008);
		mgrMonster:DelayAdd(51000027, 51000027, 51000001, 51000027, 101020180, -1, 13, dwObjId, 2500);
		for i = 1, #pos1 do
			mgrMonster:DelayAdd(51000026, 51000026, 51000001, 101300, 100710980, pos1[i].x, pos1[i].y, dwObjId, 2000);
		end
		
		for i = 1, #pos2 do
			mgrMonster:DelayAdd(51000025, 51000025, 51000001, 101300, 100820020, pos2[i].x, pos2[i].y, dwObjId, 2000);
		end
	end
...
function MapLogic:RemoveMon(map)
	local mgrMonster = map:GetMonsterMgr();		----获得怪物管理器
	local monster = {51000025, 51000026, 51000027};
	for i = 1, #monster do
