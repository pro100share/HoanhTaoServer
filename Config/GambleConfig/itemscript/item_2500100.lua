...
function ItemScriptObj:CheckForUse(objItemSys, objItem)
	...
	if not bournSystem:CheckCanAddExp(1) then 
...
function ItemScriptObj:OnUseItem(objItemSys, objItem)
		local objPlayer = objItemSys:GetPlayer()
	    --CUIRole:OpenPanel(2)
		objPlayer:GetSystem('CBournSystem'):AddBournExp(100)
	return false
end
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	local objPlayer = objItemSys:GetPlayer()
	objPlayer:GetSystem('CBournSystem'):AddBournExp(100*num)
end
-------------------------------------------------------
