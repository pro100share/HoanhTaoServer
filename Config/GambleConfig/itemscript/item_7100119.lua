...
function ItemScriptObj:CheckForUse(objItemSys, objItem,sysScript)
	...
	if math.sqrt((Pos.x-42)^2+(Pos.y-18)^2) > 5 then  sysTask:Error(999911) return nil end; 
...
function ItemScriptObj:OnUseItem(objItemSys, objItem,sysScript)	
	local objPlayer = objItemSys:GetPlayer()
	local sysTask = objPlayer:GetSystem("CTaskSystem")
	local tabAddList={{dwItemEnum=7100099,dwItemNumber=1,dwBindType=enItemBindType.eYes,}} 
