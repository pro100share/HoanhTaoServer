...
function ItemScriptObj:OnUseItem(objItemSys, objItem)
	...
	objPlayer:GetSystem('CFiveElemSystem'):FiveElementRecord(500, 500, 500, 500, 500, _G.FiveElemSysOperType.ChgFiveByUseItem)
	--金
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwMetalValue", 500) then
		bUse = true;
	else
		--dwError = 6000610021;
		isMetalFull = true;
	...
	--木
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwWoodValue", 500) then
		bUse = true;
	else
		--dwError = 6000610022;
		isWoodFull = true;
	...
	--水
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwWaterValue", 500) then
		bUse = true;
	else
		--dwError = 6000610023;
		isWaterFull = true;
	...
	--火
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwFireValue", 500) then
		bUse = true;
	else
		--dwError = 6000610024;
		isFireFull = true;
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	...
	local isFireFull = false;
	--金
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwMetalValue", 500*num) then
		bUse = true;
	else
		isMetalFull = true;
	end;
	--木
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwWoodValue", 500*num) then
		bUse = true;
	else
		isWoodFull = true;
	end;
	--水
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwWaterValue", 500*num) then
		bUse = true;
	else
		isWaterFull = true;
	end;
	--火
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwFireValue", 500*num) then
		bUse = true;
	else
		isFireFull = true;
	end;
