local dwItemId = 5500610
local ItemScriptObj = {};
setmetatable(ItemScriptObj, {__index = CItemScript});

function ItemScriptObj:CheckForUse(objItemSys, objItem, sysScript)
	if objItemSys:GetBindGold() + 8888 > _G.InitConfig.dwMaxMoney then
		return false;
	end
	return 0;
end;
...
function ItemScriptObj:OnUseItem(objItemSys, objItem)
	objItemSys:AddBindGold(8888, _G.ItemSysOperType.Used);
	return false
end
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	objItemSys:AddBindGold(8888*num, _G.ItemSysOperType.Used);
	return false
end
-------------------------------------------------------
