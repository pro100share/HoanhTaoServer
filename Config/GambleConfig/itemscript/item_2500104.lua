...
function ItemScriptObj:CheckForUse(objItemSys, objItem)
	...
	if not bournSystem:CheckCanAddExp(5) then 
		objItemSys:OnError(3000310005)
		return false;
	end
	return 0;
end
...
function ItemScriptObj:OnUseItem(objItemSys, objItem)
		local objPlayer = objItemSys:GetPlayer()
	    objPlayer:GetSystem('CBournSystem'):AddBournExp(2025)
	return false
end
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	local objPlayer = objItemSys:GetPlayer()
	objPlayer:GetSystem('CBournSystem'):AddBournExp(2025*num)
end
-------------------------------------------------------
