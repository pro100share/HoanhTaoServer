...
function ItemScriptObj:CheckForUse(objItemSys, objItem,sysScript)
 ...
 local objPlayer = objItemSys:GetPlayer()
	...
	if dwMapId ~= 5003 then sysTask:Error(999911) return nil end;
	if math.sqrt((fPosX-7)^2+(fPosY-8)^2) > 5 then sysTask:Error(999911) return nil end; 
