local dwItemId = 5929003
local ItemScriptObj = {};
setmetatable(ItemScriptObj, {__index = CItemScript});

--使用物品
function ItemScriptObj:OnUseItem(objItemSys, objItem)
	if objItemSys:GetPacketMoney() + 200000 > _G.InitConfig.dwMaxMoney then	
		return 6000610001;
	end
	objItemSys:AddPacketMoney(200000, _G.ItemSysOperType.Used);	
	return false
end
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	if objItemSys:GetPacketMoney() + 200000*num > _G.InitConfig.dwMaxMoney then
		return 6000610001;
	else
		objItemSys:AddPacketMoney(200000*num, _G.ItemSysOperType.Used);
		return false
	end
end
-------------------------------------------------------
