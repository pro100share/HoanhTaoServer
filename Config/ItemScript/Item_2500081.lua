...
function ItemScriptObj:OnUseItem(objItemSys, objItem)
	...
	if (CurBuffValue+30000) >= _G.InitConfig.dwBuffValue then
		return 3003010006;
	end
	    objPlayer:GetSystem('CBuffSystem'):AddBuff(2500080,2);
	return false;
end
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	local objPlayer = objItemSys:GetPlayer();
	objPlayer:GetSystem('CBuffSystem'):MassAddBuff(2500080,2,nil,nil,num);
end
