...
local function FreeCracker(Selfx,Selfy,Targetx,Targety,selfName,targeName,mgrNpc,MapID,objCurMap,mgrMonster,objPlayer)
	if (Selfx-Targetx)^2 + (Selfy-Targety)^2 < 25 then
		...
		local Data = mgrNpc:Add(10021127,fire_x,fire_y,5.5);     
		local mapName = GetCurMapName(MapID);
		CScriptManager:ChatNoticeAll(string.format(SysStringConfigInfo[4000120050],selfName,mapName,targeName,selfName,targeName,mapName,fire_x,fire_y,selfName),1,true)
