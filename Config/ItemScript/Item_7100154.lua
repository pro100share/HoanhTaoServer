...
function ItemScriptObj:CheckForUse(objItemSys, objItem,sysScript)
	...
	if dwMapId ~= 5081 then sysTask:Error(999911) return nil end;
	if math.sqrt((Pos.x-13)^2+(Pos.y-11)^2) > 5 then sysTask:Error(999911) return nil end; 
...
function ItemScriptObj:OnUseItem(objItemSys, objItem,sysScript)
 ...
 --[[
	local Task = sysTask:GetTask(4037) 
		if Task:GetTaskState()==TaskStateConfig.Done then
			sysScript:AddMon(10051007,23,-22)		
			sysScript:Story(91)
		end	
		--]]
end
