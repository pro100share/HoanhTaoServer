...
local dwNowTime = _now()
	...
	if math.sqrt((Pos.x-35)^2+(Pos.y+13)^2) > 5 then  sysTask:Error(999911) return nil end; 
	return 3000
...
function ItemScriptObj:OnUseItem(objItemSys, objItem,sysScript)
	...
	local sysMap = objPlayer:GetSystem("CMapSystem")
	local sysTask = objPlayer:GetSystem("CTaskSystem")
	local tabAddList={{dwItemEnum=7100041,dwItemNumber=1,dwBindType=enItemBindType.eYes,}} 
