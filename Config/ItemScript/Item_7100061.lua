...
function ItemScriptObj:CheckForUse(objItemSys, objItem,sysScript)
	...
	if dwMapId ~= 1007 then sysTask:Error(999911) return true end;
	if math.sqrt((Pos.x+32)^2+(Pos.y-91)^2) > 5 then sysTask:Error(999911) return true end; 
 return 3000
...
function ItemScriptObj:OnUseItem(objItemSys, objItem,sysScript)
	...
	local sysMap = objPlayer:GetSystem("CMapSystem")	
	local sysTask = objPlayer:GetSystem("CTaskSystem")
	local tabAddList={{dwItemEnum=7100062,dwItemNumber=1,dwBindType=enItemBindType.eYes,}} 
	...
	local Task = sysTask:GetTask(6007)		
