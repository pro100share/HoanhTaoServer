...
function ItemScriptObj:OnUseItem(objItemSys, objItem)
	...
	objPlayer:GetSystem('CFiveElemSystem'):FiveElementRecord(1000, 1000, 1000, 1000, 1000, _G.FiveElemSysOperType.ChgFiveByUseItem)
	--金
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwMetalValue", 1000) then
		bUse = true;
	else
		--dwError = 6000610021;
		isMetalFull = true;
	...
	--水
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwWaterValue", 1000) then
		bUse = true;
	else
		--dwError = 6000610023;
		isWaterFull = true;
	...
	--火
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwFireValue", 1000) then
		bUse = true;
	else
		--dwError = 6000610024;
		isFireFull = true;
	...
	--土
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwEarthValue", 1000) then
		bUse = true;
	else
		--dwError = 6000610025;
		isEarthFull = true;
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	...
	local isFireFull = false;
	--金
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwMetalValue", 1000*num) then
		bUse = true;
	else
		isMetalFull = true;
	end;
	...
	--水
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwWaterValue", 1000*num) then
		bUse = true;
	else
		isWaterFull = true;
	end;
	--火
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwFireValue", 1000*num) then
		bUse = true;
	else
		isFireFull = true;
	end;
	--土
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwEarthValue", 1000*num) then
		bUse = true;
	else
		isEarthFull = true;
	end;
