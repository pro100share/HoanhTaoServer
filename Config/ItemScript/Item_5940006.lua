...
function ItemScriptObj:OnUseItem(objItemSys, objItem)
	...
	objPlayer:GetSystem('CFiveElemSystem'):FiveElementRecord(10000, 10000, 10000, 10000, 10000, _G.FiveElemSysOperType.ChgFiveByUseItem)
	--金
	...
	--水
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwWaterValue", 10000) then
		bUse = true;
	else
		--dwError = 6000610023;
		isWaterFull = true;
	...
	--火
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwFireValue", 10000) then
		bUse = true;
	else
		--dwError = 6000610024;
		isFireFull = true;
	...
	--土
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwEarthValue", 10000) then
		bUse = true;
	else
		--dwError = 6000610025;
		isEarthFull = true;
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	...
	--木
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwWoodValue", 10000*num) then
		bUse = true;
	else
		isWoodFull = true;
	end;
	--水
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwWaterValue", 10000*num) then
		bUse = true;
	else
		isWaterFull = true;
	end;
	--火
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwFireValue", 10000*num) then
		bUse = true;
	else
		isFireFull = true;
	end;
	--土
	if objPlayer:GetSystem('CFiveElemSystem'):ChangeAttr("dwEarthValue", 10000*num) then
		bUse = true;
	else
		isEarthFull = true;
	end;
