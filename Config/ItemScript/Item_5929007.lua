local dwItemId = 5929007
local ItemScriptObj = {};
setmetatable(ItemScriptObj, {__index = CItemScript});

--使用物品
function ItemScriptObj:OnUseItem(objItemSys, objItem)
	if objItemSys:GetPacketMoney() + 5000 > _G.InitConfig.dwMaxMoney then	
		return 6000610001;
	end
	objItemSys:AddPacketMoney(5000, _G.ItemSysOperType.Used);	
	return false
end
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	if objItemSys:GetPacketMoney() + 5000*num > _G.InitConfig.dwMaxMoney then
		return 6000610001;
	else
		objItemSys:AddPacketMoney(5000*num, _G.ItemSysOperType.Used);
		return false
	end
end
-------------------------------------------------------
