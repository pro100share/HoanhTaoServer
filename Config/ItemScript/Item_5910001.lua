local dwItemId = 5910001
local ItemScriptObj = {};
setmetatable(ItemScriptObj, {__index = CItemScript});

--使用物品
function ItemScriptObj:OnUseItem(objItemSys, objItem)
	if objItemSys:GetBindGold() + 5 > _G.InitConfig.dwMaxMoney then	
		return 6000610003;
	end
	objItemSys:AddBindGold(5, _G.ItemSysOperType.Used);	
	return false
end
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	if objItemSys:GetBindGold() + 5*num > _G.InitConfig.dwMaxMoney then
		return 6000610003;
	else
		objItemSys:AddBindGold(5*num, _G.ItemSysOperType.Used);
		return false
	end
end
