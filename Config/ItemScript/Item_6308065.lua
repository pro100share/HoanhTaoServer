local dwItemId = 6308065
local ItemScriptObj = {};
setmetatable(ItemScriptObj, {__index = CItemScript});

--使用物品
function ItemScriptObj:OnUseItem(objItemSys, objItem)
	if objItemSys:GetGold() + 1000 > _G.InitConfig.dwMaxMoney then	
		return 6000610002;
	end
...
function ItemScriptObj:OnUseMassItem(objItemSys, objItem, sysScript,num)
	if objItemSys:GetGold() + 1000*num > _G.InitConfig.dwMaxMoney then
		return 6000610002;
	else
		objItemSys:AddGold(1000*num, _G.ItemSysOperType.Used);
		return false
	end
end
-------------------------------------------------------
