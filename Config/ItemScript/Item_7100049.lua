...
local dwNowTime = _now()
	...
	if math.sqrt((Pos.x-96)^2+(Pos.y+6)^2) > 5 then sysTask:Error(999911) return nil end; 
...
function ItemScriptObj:OnUseItem(objItemSys, objItem,sysScript)
	...
	local tabAddList={{dwItemEnum=7100050,dwItemNumber=1,dwBindType=enItemBindType.eYes,}} 
