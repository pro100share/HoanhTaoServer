...
local IsWindows = true--string.find(string.lower(os.getenv('OS') or ''),'^windows')
...
local function gl( level, j )
  if not debug.getinfo(level+1) then
...
function debug.dump(v,depth)
  dumpvar(v,(tonumber(depth) and depth or debug.dumpdepth)+1,tostring(v))
end
...
function debug.pausetraceback(pause)
	if pause then
		debug.traceback = clitraceback
	else
