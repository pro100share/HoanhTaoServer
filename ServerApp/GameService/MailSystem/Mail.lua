...
function CMail:SetAccessory( dwGold, dwBindGold, dwMoney, setItems )
	...
	for k,objItem in pairs(setItems) do
		...
		self.sInfo["dwItemID"..dwCount] = json.encode(objItem);
		objItem.dwItemID = dwID;	--物品的新ID
		objItem.dwRoleID = 0;		--物品所属邮件的ID
		objItem.dwBornTime = GetCurTime();---物品的出生时间以领取的时间为起点
		if dwCount >= 5 then
			break;
		end
	end
...
function CMail:CheckLoadItem()
	for k=1,5 do
		if self.sInfo["dwItemID"..k] ~= 0 then
...
function CMail:LoadItem( index,str )
	...
	local selStr = 'select * from "T_Role_Item" where "dwRoleID" = $1 and "dwItemID" = $2 limit 1';
